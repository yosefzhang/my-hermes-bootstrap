...
- 必须读取 `my-hermes-bootstrap.env`（可由 `script/generate_bootstrap_env.py` 自动生成，也可手动从 `my-hermes-bootstrap.env.template` 复制编辑），确保所有必需的变量都已正确设置。
...
仅在 `HTTP_PROXY` 已设置时才配置代理，否则跳过。

```bash
source ~/.hermes/.env
if [ -n "$HTTP_PROXY" ]; then
  hermes config set proxy.http "$HTTP_PROXY"
  hermes config set proxy.https "${HTTPS_PROXY:-$HTTP_PROXY}"
  hermes config set proxy.no_proxy "${NO_PROXY:-localhost,127.0.0.1,10.0.0.0/8,::1}"
fi
```
...
### Step 10.5: 配置审批策略与界面语言

写入审批和界面语言从环境变量读取，默认为当前推荐配置：

```bash
source ~/.hermes/.env
hermes config set memory.write_approval "${MEMORY_WRITE_APPROVAL:-true}"
hermes config set skills.write_approval "${SKILLS_WRITE_APPROVAL:-true}"
hermes config set display.language "${DISPLAY_LANGUAGE:-zh}"
```
...
## 参考文件
- `my-hermes-bootstrap.env` — 当前环境的配置文件快照（由脚本生成）
- `my-hermes-bootstrap.env.template` — 环境变量模板（纯 .env 格式）
- `resource/` — Hermes 未安装时的回退来源（`.env` + `config.yaml`）
- `script/generate_bootstrap_env.py` — 自动从 Hermes 安装或 `resource/` 回退生成 env
